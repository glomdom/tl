...
template <>
struct std::formatter<tlc::core::lex::TokenType> : std::formatter<std::string_view> {
  auto format(const tlc::core::lex::TokenType type, auto& ctx) const {
    return std::formatter<std::string_view>::format(tlc::core::lex::to_view(type), ctx);
  }
};
